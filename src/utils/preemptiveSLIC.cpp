    ...
    if(init_labels_flag)
    {
      ...
      for(int i=0; i<m_ny;++i)
      {
        ...
        {      
          ...
          // set initial labels
          for(int ii=round(cur_y-m_pixel_offset-m_sy/2); ii<round(cur_y-m_pixel_offset+m_sy/2); ++i)
            for(int jj=round(cur_x-m_pixel_offset-m_sx/2); jj<round(cur_x-m_pixel_offset+m_sx/2); ++j)
...
void PreemptiveSLIC::preemptiveSLIC(const Mat& I_rgb, const int k, const double compactness, Mat& labels, Mat& seeds)
{
  ...
  for(int y=0; y<I_lab.rows;y++)
  {
    for(int x=0;x<I_lab.cols;x++)
    {
      ...
      cv::Vec3b &lab = I_lab.at<cv::Vec3b>(y,x);
      m_lvec[idx] = (double)lab[0];
      m_avec[idx] = (double)lab[1];
      m_bvec[idx] = (double)lab[2];
